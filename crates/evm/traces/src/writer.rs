// Local copy of `revm-inspectors` 0.42.2's tracing writer, licensed under
// MIT OR Apache-2.0. It is mechanically aligned with the upstream writer
// except for CPU metrics in call headers.
#![allow(clippy::collapsible_if, clippy::missing_const_for_fn)]

use crate::{CpuTraceMetrics, cpu::format_duration};
...
/// Configuration for a [`FoundryTraceWriter`].
#[derive(Clone, Debug)]
#[allow(missing_copy_implementations)]
pub struct FoundryTraceWriterConfig {
    use_colors: bool,
...
impl Default for FoundryTraceWriterConfig {
    fn default() -> Self {
        Self::new()
    }
}

impl FoundryTraceWriterConfig {
    /// Create a new `FoundryTraceWriterConfig` with default settings.
    pub fn new() -> Self {
        Self {
...
#[derive(Clone, Debug)]
pub struct FoundryTraceWriter<W> {
    writer: W,
    indentation_level: u16,
    config: FoundryTraceWriterConfig,
}

impl<W: Write> FoundryTraceWriter<W> {
    /// Create a new `FoundryTraceWriter` with the given writer.
    #[inline]
    pub fn new(writer: W) -> Self {
        Self::with_config(writer, FoundryTraceWriterConfig::new())
    }

    /// Create a new `FoundryTraceWriter` with the given writer and configuration.
    pub fn with_config(writer: W, config: FoundryTraceWriterConfig) -> Self {
    ...
    /// Consumes the `FoundryTraceWriter` and returns the inner writer.
    #[inline]
    pub fn into_writer(self) -> W {
        self.writer
    }

    /// Writes a call trace arena to the writer, optionally annotating call headers with CPU data.
    pub fn write_arena(
        &mut self,
        arena: &CallTraceArena,
        metrics: Option<&CpuTraceMetrics>,
    ) -> io::Result<()> {
        self.write_node(arena.nodes(), 0, metrics)?;
        self.writer.flush()
    }
    ...
    fn write_item(
        ...
        match &node.ordering[item_idx] {
            ...
            TraceMemberOrder::Call(index) => {
                self.write_node(nodes, node.children[*index], metrics)?;
                Ok(item_idx + 1)
            }
            TraceMemberOrder::Step(index) => {
                self.write_step(nodes, node_idx, item_idx, *index, metrics)
            }
        }
    }
    ...
    fn write_items_until(
        ...
        while !f(item_idx) {
            item_idx = self.write_item(nodes, node_idx, item_idx, metrics)?;
        }
        Ok(item_idx)
    }
    ...
    fn write_items(
        ...
        self.write_items_until(nodes, node_idx, 0, |idx| idx == items_cnt, metrics)?;
        Ok(())
    }
    ...
    fn write_node(
        ...
        // Write header.
        self.write_branch()?;
        self.write_trace_header(&node.trace, idx, metrics)?;
        self.writer.write_all(b"\n")?;

        // Write logs and subcalls.
        self.indentation_level += 1;
        self.write_items(nodes, idx, metrics)?;
    ...
    fn write_trace_header(
        &mut self,
        trace: &CallTrace,
        node_idx: usize,
        metrics: Option<&CpuTraceMetrics>,
    ) -> io::Result<()> {
        match metrics.and_then(|all| all.get(&node_idx)) {
            Some(cpu) => write!(
                self.writer,
                "[{} gas | {} cpu | {} self | {:.2}%] ",
                trace.gas_used,
                format_duration(cpu.inclusive_ns),
                format_duration(cpu.self_ns),
                f64::from(cpu.root_percent_bps) / 100.0,
            )?,
            None => write!(self.writer, "[{}] ", trace.gas_used)?,
        }
        ...
        if trace.kind.is_any_create() {
            ...
            let (func_name, inputs) =
                match trace.decoded.as_ref().and_then(|decoded| decoded.call_data.as_ref()) {
    ...
    fn write_log(&mut self, log: &CallLog) -> io::Result<()> {
        ...
        if let Some(name) = log.decoded.as_deref().and_then(|decoded| decoded.name.as_deref()) {
            write!(self.writer, "emit {name}({log_style}")?;
            if let Some(params) =
                log.decoded.as_deref().and_then(|decoded| decoded.params.as_deref())
            {
    ...
    fn write_trace_footer(&mut self, trace: &CallTrace) -> io::Result<()> {
        ...
        if let Some(decoded) =
            trace.decoded.as_ref().and_then(|decoded| decoded.return_data.as_deref())
        {
            write!(self.writer, " ")?;
...
mod tests {
    use super::*;
    use crate::CallTraceCpuMetrics;

    #[test]
    fn writer_without_metrics_matches_upstream_exactly() {
        let arena = arena();
        let mut upstream = revm_inspectors::tracing::TraceWriter::new(Vec::new());
        upstream.write_arena(&arena).unwrap();

        let mut local = FoundryTraceWriter::new(Vec::new());
        local.write_arena(&arena, None).unwrap();

        assert_eq!(local.into_writer(), upstream.into_writer());
    }

    #[test]
    fn writer_includes_cpu_metrics_in_call_header() {
        let arena = arena();
        let metrics = CpuTraceMetrics::from([(
            0,
            CallTraceCpuMetrics { inclusive_ns: 1_500, self_ns: 1_000, root_percent_bps: 10_000 },
        )]);
        let mut writer = FoundryTraceWriter::new(Vec::new());

        writer.write_arena(&arena, Some(&metrics)).unwrap();

        let rendered = String::from_utf8(writer.into_writer()).unwrap();
        assert!(rendered.starts_with("  [42 gas | 1.5us cpu | 1.0us self | 100.00%] "));
    }

    fn arena() -> CallTraceArena {
        let mut arena = CallTraceArena::default();
        arena.nodes_mut()[0].trace = CallTrace { gas_used: 42, ..Default::default() };
        arena
    }
}
